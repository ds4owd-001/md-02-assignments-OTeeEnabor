...
```{r}
#| label: load-packages
library(gapminder) #dataset package
...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    -   Continent - a categorical character (fct)

    -   Life expectancy - continuous numerical (dbl)
...
ggplot(gapminder_2007,
  ...
  labs(
    title = "Continent and Life Expectancy",
    x = "Continent",
...
2.  Use the `ggplot()` function and the `gapminder_summary_continent_year` object to create a connected scatter plot (also called time series plot) using the `geom_line()` and `geom_point()`functions with the following aesthetic mappings:
...
ggplot(gapminder_summary_continent_year,
  ...
  geom_point() + 
  labs (
    title = "Year and Life Expectancy by Continent",
    x = "Year",
    y = "Life Expectancy (years)"
...
## Task 8: Create a bar-plot

### with `geom_col()`

1.  Add a new code chunk below.

2.  Use the `ggplot()` function and the `gapminder_summary_2007` object to create a bar-plot using the `geom_col()` function with the following aesthetic mappings:
...
ggplot(gapminder_summary_2007,
  ...
  geom_col() +
  labs (
    title = "Number of Observations per Continent",
    x = "Continent",
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    The geom_bar() function performs the counting of each categorical object automatically - does not need a y parameter unlike the geom_col() function.
...
***The bin boundaries change depending on the sequence of values given to the breaks argument.***
...
ggplot(gapminder_2007,
  ...
  labs (
    title = "Life Epectanct and GDP per Capita per Continent",
    x = "GDP per Capita",
    y = "Life Expectancy (years)"
...
ggplot(data = gapminder,
  ...
  theme_minimal() +
  labs (
    title = "Life Expectancy by Year per Continent",
    x = "Year",
    y = "Life Expectancy"
